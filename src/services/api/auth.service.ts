...
import { IAuth } from '@Emmanuel-MacAnThony/jobber-shared';
...
class AuthService {
  ...
  async getGigs(query: string, from: string, size: string, type: string): Promise<AxiosResponse> {
    const response: AxiosResponse = await this.axiosService.axiosInstance.get(`/search/gig/${from}/${size}/${type}?${query}`);
    return response;
  }

  async getGig(gigId: string): Promise<AxiosResponse> {
    const response: AxiosResponse = await this.axiosService.axiosInstance.get(`/search/gig/${gigId}`);
    return response;
  }
